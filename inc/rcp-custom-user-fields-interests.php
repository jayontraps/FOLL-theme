 ...
 *
 */
function pw_rcp_add_interests_fields_to_reg_form() {
...
add_action( 'rcp_after_password_registration_field', 'pw_rcp_add_interests_fields_to_reg_form' );
 ...
 *
 */
function pw_rcp_add_interests_fields() {
...
add_action( 'rcp_profile_editor_after', 'pw_rcp_add_interests_fields' );
 ...
 *
 */
function pw_rcp_add_interests_member_edit_fields( $user_id = 0 ) {
...
add_action( 'rcp_edit_member_after', 'pw_rcp_add_interests_member_edit_fields' );
...
function pw_rcp_save_interests_fields_on_register( $posted, $user_id ) {

	if( ! empty( $posted['rcp_interests'] ) ) {
		update_user_meta( $user_id, 'rcp_interests', sanitize_text_field( $posted['rcp_interests'] ) );
	}
}
add_action( 'rcp_form_processing', 'pw_rcp_save_interests_fields_on_register', 10, 2 );
 ...
 *
 */
function pw_rcp_save_interest_fields_on_profile_save( $user_id ) {

	if( ! empty( $_POST['rcp_interests'] ) ) {
		update_user_meta( $user_id, 'rcp_interests', sanitize_text_field( $_POST['rcp_interests'] ) );
	}
}
add_action( 'rcp_user_profile_updated', 'pw_rcp_save_interest_fields_on_profile_save', 10 );
add_action( 'rcp_edit_member', 'pw_rcp_save_interest_fields_on_profile_save', 10 );