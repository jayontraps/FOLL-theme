...
$(document).ready(function() {
	...
	// var table = $('#results');	
	// var url = "http://localhost/foll/wp-content/uploads/2015/06/dataset.csv";	
	// var url = datasetUrl;
	var search = "";
	...
	// build form els and format date 
	var buildFormEls = function(results){
		...
		for (var i = 0; i < locationArray.length; i++) {
			$('<option value="' + locationArray[i] + '">' + locationArray[i] + '</option>').appendTo(locList);
		}
	...
	var buildDomEls = function(totalResults, resultsArr) {
		...
		for (var i = 0; i < totalResults; i++) {
			$('<tr/>', {
				html : '<td class="species">'  + resultsArr[i].species + '</td> '  + '<td class="date">'  + resultsArr[i].date_from + '</td> '  + '<td class="location">' + resultsArr[i].location + '</td> '  + '<td class="count">'  + resultsArr[i].count + '</td> ' + '<td class="observer">'  + resultsArr[i].observer + '</td> ' + '<td class="notes">'  + resultsArr[i].notes + '</td>'
				}						
			).appendTo(newRows);			
		}
	...
	var displayResults = function(resultsArr, theSearch) {
		...
		// $(searchSummary).html(readBackResults);
...
var arrivalAndDeparture = function(results, theRange, theSeason) {
	...
	for (counter = 0; counter < finalArrLen; counter++) {
		$('<tr/>', {
			html : '<td class="date">'  + arrBySeason[counter].date_from + '</td> '  + '<td class="location">' 					 + arrBySeason[counter].location + '</td> '  + '<td class="count">'  + arrBySeason[counter].count + '</td> ' + '<td class="observer">'  + arrBySeason[counter].observer + '</td> ' + '<td class="notes">'  + arrBySeason[counter].notes + '</td>'
			}						
		).appendTo(newRows);			
	}
